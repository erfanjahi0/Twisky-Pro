    ...
    switch (action) {
      ...
      case 'upload_image': {
        ...
        for (const img of Object.values(r.data?.images || {})) {
          if ((img as any).hash) { hash = (img as any).hash; }
          if ((img as any).url) { imageUrl = (img as any).url; }
          if (hash) break;
        }
        result = hash ? { success: true, hash, image_url: imageUrl } : { success: false, error: 'No hash returned' };
        break;
      }
      ...
      case 'publish_post': {
        // Immediate publish: update the existing story to is_published=true
      ...
      case 'schedule_post': {
        // Scheduled publish via the Page Feed API.
        // The correct Meta approach: POST to /{page-id}/feed with:
        //   published=false  +  scheduled_publish_time=<unix_timestamp>
        // This is completely separate from the ad-creative flow — scheduling
        // cannot be done by updating an existing story_id.
        const { page_id, page_token, image_url, caption, headline, destination_url, scheduled_publish_time } = payload;

        if (!page_id) { result = { success: false, error: 'No page_id provided' }; break; }
        if (!scheduled_publish_time) { result = { success: false, error: 'No scheduled_publish_time provided' }; break; }

        const postData: Record<string, string> = {
          access_token: page_token || token,
          published: 'false',
          scheduled_publish_time: String(scheduled_publish_time),
          message: caption || headline || '',
        };

        // Attach link if destination URL is provided
        if (destination_url) postData['link'] = destination_url;

        // Attach picture URL if available (retrieved from adimages after upload)
        if (image_url) postData['picture'] = image_url;

        const r = await fbRequest(`${FB_API}/${page_id}/feed`, c_user, xs, postData);
        if (!r.success) { result = { success: false, error: r.error }; break; }
        const postId = r.data?.id || null;
        result = postId
          ? { success: true, post_id: postId }
          : { success: false, error: 'No post ID returned from Facebook' };
        break;
      }

      case 'get_image_url': {
        // Fetch the public URL of an uploaded ad image by its hash
        const { ad_account_id, image_hash, page_token } = payload;
        if (!ad_account_id || !image_hash) { result = { success: false, error: 'Missing ad_account_id or image_hash' }; break; }
        const r = await fbRequest(
          `${FB_API}/act_${ad_account_id}/adimages?hashes[]=${encodeURIComponent(image_hash)}&fields=url&access_token=${encodeURIComponent(page_token || token)}`,
          c_user, xs
        );
        if (!r.success) { result = { success: false, error: r.error }; break; }
        const imgUrl = r.data?.data?.[0]?.url || null;
        result = imgUrl ? { success: true, image_url: imgUrl } : { success: false, error: 'Could not retrieve image URL' };
        break;
      }
      ...
      case 'verify_post': {
        ...
        const r = await fbRequest(
          `${FB_API}/${post_id}?fields=id,scheduled_publish_time,is_published&access_token=${encodeURIComponent(page_token || token)}`,
          c_user, xs
        );
