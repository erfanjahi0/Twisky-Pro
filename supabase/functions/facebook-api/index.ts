...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'publish_post': {
        const { story_id, page_token, published, scheduled_publish_time } = payload;
        const isScheduled = published === false && scheduled_publish_time;
        ...
        if (isScheduled) {
          // Scheduled post: Meta requires published=false + scheduled_publish_time (Unix timestamp)
          postFields['published'] = 'false';
          postFields['scheduled_publish_time'] = String(scheduled_publish_time);
        } else {
          postFields['is_published'] = 'true';
        }

        const r = await fbRequest(`${FB_API}/${story_id}`, c_user, xs, postFields);
        if (!r.success) { result = { success: false, error: r.error }; break; }
